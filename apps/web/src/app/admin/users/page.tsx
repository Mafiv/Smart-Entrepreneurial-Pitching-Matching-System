...
import { PenLine, BarChart3, MessageSquare, User as UserIcon } from "lucide-react";
...
function DocLink({ url, label, missing }: { url?: string; label: string; missing?: string }) {
	if (!url) {
		return (
			<div className="flex items-center gap-3 rounded-xl border-2 border-dashed border-border/40 bg-muted/10 p-4">
				...
				<div>
					...
					<p className="text-xs text-muted-foreground/40">{missing || "Not uploaded"}</p>
				</div>
			</div>
		);
	}

	const isImage = url.match(/\.(jpg|jpeg|png|webp|gif)/i) || url.includes("/image/upload/");
	...
	return (
		<div className="rounded-xl border border-border/50 overflow-hidden bg-card">
			<div className="flex items-center justify-between px-4 py-2.5 bg-muted/30 border-b border-border/30">
				...
				<a href={url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline flex items-center gap-1 shrink-0">
			...
			{isImage ? (
				<div className="bg-[repeating-conic-gradient(hsl(var(--muted))_0%_25%,transparent_0%_50%)] bg-[length:16px_16px] p-3 flex items-center justify-center">
					<img src={url} alt={label} className="max-h-[240px] w-auto rounded-lg object-contain shadow-sm border border-border/20" />
				</div>
			) : (
...
export default function AdminUsersPage() {
	...
	const handleStatusUpdate = async (overrideStatus?: string, overrideReason?: string) => {
		...
		try {
			...
			await fetch(`${api}/auth/admin/users/${actionUser._id}/status`, {
				...
				body: JSON.stringify({
					status: statusToSet,
					reason: statusToSet === "unverified" ? (overrideReason || rejectionReason) : undefined,
				}),
			});
	...
	return (
		<ProtectedRoute allowedRoles={["admin"]}>
			<DashboardLayout navItems={ADMIN_NAV} title="SEPMS Admin">
					...
					<Table>
						...
						<TableBody>
							{loading ? (
								...
								filteredUsers.map((u) => {
									const isProtected = u.adminLevel === "super_admin" && !isSuperAdmin;
									return (
									<TableRow key={u._id} className={isProtected ? "" : "cursor-pointer hover:bg-muted/50"} onClick={() => {
										...
										<TableCell>
											<Badge variant={roleBadge(u.role)} className="text-xs capitalize gap-1">
												{u.adminLevel === "super_admin" && <Crown className="h-3 w-3 text-amber-500" />}
												{u.adminLevel === "super_admin" ? "Super Admin" : u.role}
											</Badge>
										</TableCell>
										<TableCell>
											<Badge variant={statusBadge(u.status)} className="text-xs capitalize">
												{u.status}
											</Badge>
										</TableCell>
										<TableCell>
				...
				<Dialog open={!!actionUser} onOpenChange={() => {
					setActionUser(null);
					setActionUserProfile(null);
					setRejectionReason("");
				}}>
							...
							<DialogHeader>
								<DialogTitle className="flex items-center gap-2">
									{actionUser?.status === "pending" ? (
										<><ShieldCheck className="h-5 w-5 text-primary" /> KYC Review</>
									) : (
										<><Users className="h-5 w-5" /> Manage User</>
									)}
								</DialogTitle>
								{actionUser?.status === "pending" && (
									<DialogDescription>Review the submitted KYC documents and approve or reject this user.</DialogDescription>
								)}
							</DialogHeader>
						</div>
									...
									<div className="rounded-xl bg-muted/30 border border-border/50 p-4">
										<div className="flex items-center gap-4">
											...
											<div className="flex-1 min-w-0">
												<p className="font-semibold text-base">{actionUser.fullName}</p>
												<p className="text-sm text-muted-foreground">{actionUser.email}</p>
												<div className="flex gap-2 mt-2">
													<Badge variant={roleBadge(actionUser.role)} className="text-xs capitalize gap-1">
														{actionUser.adminLevel === "super_admin" && <Crown className="h-3 w-3 text-amber-500" />}
														{actionUser.adminLevel === "super_admin" ? "Super Admin" : actionUser.role}
													</Badge>
													<Badge variant={statusBadge(actionUser.status)} className="text-xs capitalize">{actionUser.status}</Badge>
												</div>
											</div>
										</div>
									</div>
									...
									<div className="space-y-3">
										...
										{loadingProfile ? (
											<div className="flex items-center justify-center py-8">
												...
												<p className="ml-2 text-sm text-muted-foreground">Loading documents...</p>
											</div>
										) : actionUserProfile && Object.keys(actionUserProfile).length > 0 ? (
											<div className="space-y-3">
												<DocLink url={actionUserProfile.nationalIdUrl} label="Ethiopian National ID" missing="National ID not uploaded" />
												{actionUser.role === "entrepreneur" && (
													<>
														<DocLink url={actionUserProfile.businessLicenseUrl} label="Business License" missing="Business license not uploaded" />
														<DocLink url={actionUserProfile.tinNumber} label="TIN Certificate" missing="TIN certificate not uploaded" />
													</>
												)}
												{actionUser.role === "investor" && (
													<DocLink url={actionUserProfile.accreditationDocumentUrl} label="Financial Accreditation" missing="Accreditation not uploaded" />
												)}
											</div>
										) : (
											<div className="rounded-lg border border-dashed p-4 text-center">
												<AlertCircle className="h-5 w-5 text-muted-foreground/50 mx-auto mb-2" />
												<p className="text-sm text-muted-foreground">No profile or documents found.</p>
											</div>
										)}
									</div>
										...
										<>
											...
											<div className="space-y-3">
												...
												<div className="grid grid-cols-2 gap-3">
													<Button className="gap-2" onClick={() => handleStatusUpdate("verified")}>
														<ShieldCheck className="h-4 w-4" /> Approve KYC
													</Button>
													<Button variant="destructive" className="gap-2" onClick={() => {
														if (!rejectionReason.trim()) { setNewStatus("reject-prompt"); return; }
												...
												{(newStatus === "reject-prompt" || rejectionReason) && (
													<div className="space-y-2">
														...
														{newStatus === "reject-prompt" && (
															<Button variant="destructive" size="sm" disabled={!rejectionReason.trim()}
																onClick={() => handleStatusUpdate("unverified", rejectionReason)}>
																Confirm Rejection
															</Button>
														)}
													</div>
									...
									{actionUser.status !== "pending" && (
										<>
											...
											<div className="space-y-2">
												...
												{newStatus === "unverified" && actionUser.status !== "unverified" && (
													<Textarea
